# -*- coding: utf-8 -*-
import traceback, ctypes, sys, glob, serial, configparser, time, logging
try:
    ...
    from PyQt5.QtCore import pyqtSignal, pyqtSlot, QThread, QThreadPool, QRunnable, QObject, QCoreApplication
...
#from Backend.Children import Target_Child as Target
...
class Radiography(object):
    ...
    def gettablecoords(self):
        ...
        x, okx = QInputDialog.getDouble(GUI, 'Table position: X', 'x_table:', 
                                        0.0, decimals = 4)
        y, oky = QInputDialog.getDouble(GUI, 'Table position: Y', 'y_table:', 
                                        0.0, decimals = 4)

        if not okx or not oky:
            GUI.TableTxt_x.setText('X Value not set!!')
            GUI.TableTxt_y.setText('Y Value not set!!')
        else:            
            GUI.TableTxt_x.setText('{:2.4f}'.format(x))
            GUI.TableTxt_y.setText('{:2.4f}'.format(y))
        
        GUI.TableTxt_x.setStyleSheet("color: #b1b1b1;")
        GUI.TableTxt_y.setStyleSheet("color: #b1b1b1;")
...
#        TxtRGShiftX
        ...
        # Caution: Head first supine and feet first prone positions have inverted dorsal-ventral and inferior-superior axes!!!
        if self.PatientPosition == 'HFS':
            target2iso_x = (-1.0)*( x_iso - x_target) + x_table + x_repo # (-1) because x-coordiantes are inverse in table/CT coordinates
            target2iso_y = (-1.0)*( y_iso - y_target) + y_table + y_repo# (-1) because y-coordiantes are inverse in table/CT coordinates
        ...
        # Highlight result
        GUI.Group_Result.setStyleSheet(".QGroupBox { border: 2px solid rgb(0,0,255);}")
    ...
    def return_spacing(self, Spacing):
        ...
        GUI.TxtRG_pxcalc.setText('Pixel Spacing: ' + '{:4.2f}'.format(self.pixelsizeRG) + 'mm')
...
class MotorControl(object):
    ...
    def __init__(self):
        ...
        # Set status boxes
        GUI.BoxTableCOM.setStyleSheet("background-color: red;")
        GUI.BoxTableInit.setStyleSheet("background-color: red;")
        GUI.BoxTableLimits.setStyleSheet("background-color: red;")
...
#            # write to Axis    
    ...
    def update_state(self, State):
        ...
        if State[0]:
            GUI.BoxTableCOM.setStyleSheet("background-color: green;")
            GUI.LabelCOM.setText('Connected')
        else:
            GUI.BoxTableCOM.setStyleSheet("background-color: red;")
            GUI.LabelCOM.setText('Disconnected')
            
        if State[1]:
            GUI.BoxTableInit.setStyleSheet("background-color: green;")
            GUI.LabelINIT.setText('Ready')
        else:
            GUI.BoxTableInit.setStyleSheet("background-color: red;")
            GUI.LabelINIT.setText('Not Ready')
        ...
        if State[2]:
            ...
            GUI.BoxTableLimits.setStyleSheet("background-color: red;")
            GUI.LabelREF.setText('Not Calibrated')        
...
#        if a == -1: 
#            self.ctrl.close()
#            GUI.Button_MotorInit.setEnabled(True)
...
#            return -1
        ...
        if State_Master.startswith('R') and State_Slave.startswith('R'):
            ...
            GUI.BoxTableInit.setStyleSheet("background-color: green;")
            GUI.LabelINIT.setText('Initialized')
